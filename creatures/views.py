...
from .models import Organism, Habitat, Diet, Behavior, Media, Reference

def organism_list(request):
    organisms = Organism.objects.all()
    context = {'organisms': organisms}
    return render(request, 'creatures/main.html', context)
...
def organism_detail(request, organism_id):
    ...
    behavior = Behavior.objects.get(organism_id=organism_id)
    media = Media.objects.get(organism_id=organism_id)
    reference = Reference.objects.filter(organism_id=organism_id)
    context = {'organism': organism, 'habitat': habitat, 'diet': diet, 'behavior': behavior, 'media': media, 'reference': reference}
    return render(request, 'creatures/organism_detail.html', context)
